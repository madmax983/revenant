...
- **Pop (Rollback Phase)**: If one branch fails, the engine transitions the instance to `Compensating`. Sibling branches that are in-flight or pending are marked `Cancelled` (and therefore are never compensated). The engine then pops completed branch steps from `Compensation_Stack__c` one by one in LIFO (Last-In-First-Out) order, executing their `compensate()` methods in the exact reverse order of their forward completion.

...
- **A bulk read never reports a step-bearing instance as empty.** The row scan shares a batch budget, so a very large instance can starve a sibling of retrieved rows — but the authoritative `COUNT()` still gives every instance its real `totalCount`, so a starved instance comes back with `isTruncated = true` and its true `totalCount` (with whatever rows were retrieved, possibly none), never as a false-empty history. To get the full `entries` of such a history, re-read that instance singly.

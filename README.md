...
Issue #125 asked for a bespoke "pending approvals inbox" LWC and a least-privilege permission set so a business approver could clear a gate without holding `Workflow_Admin`. Salesforce's **native Approval Process** already ships an approver inbox (Approvals Home, the Approvals related list), mobile approve/reject, and delegation — so Revenant leans on it instead of rebuilding it. [`NativeApprovalWorkflowExample`](examples/main/default/classes/NativeApprovalWorkflowExample.cls) is the copyable reference for wiring the two together, with **zero new UI** and the only custom Apex being two small, framework-pattern invocable actions:

1. A record-triggered Flow ([`Revenant_Start_Approval_Workflow_Example`](examples/main/default/flows/Revenant_Start_Approval_Workflow_Example.flow-meta.xml)) starts the workflow when an `Approval_Request__c` is created, calling the already-shipped `WorkflowStartInvocableAction`, persists the resolved correlation key back onto the record, then submits the record into the native Approval Process via [`NativeApprovalRequestSubmitInvocableAction`](examples/main/default/classes/NativeApprovalRequestSubmitInvocableAction.cls) — meeting a process's entry criteria is not by itself sufficient to enter a record into it; Salesforce always requires an explicit submission action, and this invocable is it.
2. The Approval Process ([`Revenant_Native_Approval_Example`](examples/main/default/approvalProcesses/Approval_Request__c.Revenant_Native_Approval_Example.approvalProcess-meta.xml)) evaluates the submitted record. Its final approval/rejection actions are plain field updates to `Approval_Status__c` — no code.
3. A second record-triggered Flow ([`Revenant_Approval_Decision_To_Signal_Example`](examples/main/default/flows/Revenant_Approval_Decision_To_Signal_Example.flow-meta.xml)) fires when that field update lands, reads the approver's actual decision comment from the standard approval history (`ProcessInstance` / `ProcessInstanceStep.Comments` — Salesforce's built-in Approve/Reject comment box is stored there, not written back onto the record automatically), and calls [`NativeApprovalDecisionSignalInvocableAction`](examples/main/default/classes/NativeApprovalDecisionSignalInvocableAction.cls), which `JSON.serialize()`s the decision payload (safe against newlines, quotes, or backslashes in the approver's comment — a hand-assembled Flow-formula string is not) and hardcodes the target signal name to `NativeApprovalWorkflowExample.DECISION_SIGNAL` so the two can never silently drift apart, resuming the suspended gate exactly as described in §5.

The gate is left unrestricted (`waitForApproval(APPROVAL_KEY, null)`). Be precise about what that does and doesn't buy you: the native Approval Process's approver assignment controls who is *prompted* to decide, but the signal path itself (`WorkflowEngine.signal`, and both invocable actions above) performs **no authorization check of its own** — this is not new to this example, it's the same property every unrestricted `waitForApproval(key, null)` gate in this repo has (see `ApprovalWorkflowExample`). If you need an actual enforced check on who may publish the decision signal, pass a shipped Custom Permission (e.g. `Workflow_Admin`) as `waitForApproval`'s second argument so `WorkflowDashboardController.enforceApprovalRole` gates it. No new Custom Permission or permission set ships with this example — that enforcement is opt-in, not a claim this example makes on your behalf.

Revenant still contributes what a native Approval Process cannot express on its own: durable multi-step orchestration and saga rollback on rejection (`ReserveBudgetStep` unwinds via `Compensation_Stack__c` exactly as in `ApprovalWorkflowExample`). The Approval Process metadata ships `active=false` (as all Approval Processes must) and the approver assignment (`relatedUserField`/`Manager`) is a placeholder — adjust it for your org before activating. `NativeApprovalWorkflowExampleTest` proves the durable round-trip in pure Apex by injecting the exact signal payload the bridge Flow publishes, so the suite has no dependency on the Approval Process being active; `NativeApprovalDecisionSignalInvocableActionTest` additionally proves the JSON-serialization path against comments containing line breaks, quotes, and backslashes.

### 5c. An alternative: Apex-trigger bridge

§5b's Flow-based bridge is admin-reconfigurable and code-free, but the Flow layer is also where nearly every review comment on that pattern landed — missing target records, missing bulk calls, `Draft` flows that silently never fire — because Flow metadata is XML you write by hand, not code the compiler checks. [`NativeApprovalTriggerBridgeExample`](examples/main/default/classes/NativeApprovalTriggerBridgeExample.cls) is the same idea with the Flow layer replaced by a single Apex trigger, driving the **exact same** `NativeApprovalWorkflowExample` workflow, keyed by `Case.Id` instead of a dedicated custom object:

- [`CaseApprovalBridgeTrigger`](examples/main/default/triggers/CaseApprovalBridgeTrigger.trigger) delegates to the handler on `after insert` (start the workflow via `WorkflowEngine.startOrGet`, then submit only the genuinely-new instances into the native Approval Process via bulk `Approval.process(List<Approval.ProcessSubmitRequest>)`) and `after update` (read the approver's comment from `ProcessInstance`/`ProcessInstanceStep.Comments` via plain bulk SOQL, then signal via bulk `WorkflowEngine.signal(List<SignalRequest>)`).
- No Flow metadata at all — no formulas, no `decisions`, no `recordLookups`, no XML shape to get subtly wrong. The input payload is a native `Map<String, Object>` instead of a hand-assembled JSON-formula string, and `Trigger.new` is bulk by construction rather than something a Flow author has to remember to batch.
- The whole bridge is testable end-to-end with real `insert`/`update` DML in an Apex test ([`NativeApprovalTriggerBridgeExampleTest`](examples/main/default/classes/NativeApprovalTriggerBridgeExampleTest.cls)) — something the Flow layer in §5b can't do, since Flow logic isn't invokable from a plain Apex unit test. That's the one thing this version demonstrably gets right that the other can't prove at all.

The trade-off is real, not just "the Flow version is worse": §5b can be rewired by an admin in Flow Builder without touching code; this version needs an Apex developer to touch a trigger and a class. Pick whichever fits your org's skill mix — both drive the same workflow definition, so neither is more "correct" than the other.

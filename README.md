...
- **Capture-Once Local Values (`once()`)**: Stabilizes values generated _locally_ inside a step — `Datetime.now()`, generated UUIDs, Crypto-random tokens, reference numbers, random branch decisions — so they do not drift across retries, yield/resume cycles, at-least-once event resumes, or operator re-drives. `StepContext.once(key, producer)` invokes the producer at most once per `(instance, step, key)` and returns the same durably recorded value on every subsequent re-execution. Contrast with `idempotencyKey`: use `idempotencyKey` to make an **external API call** effective-once; use `once()` to keep a **locally generated value** stable. See [CaptureOnceWorkflowExample](examples/main/default/classes/CaptureOnceWorkflowExample.cls).

...
1. A record-triggered Flow ([`Revenant_Start_Approval_Workflow_Example`](examples/main/default/flows/Revenant_Start_Approval_Workflow_Example.flow-meta.xml)) starts the workflow when an `Approval_Request__c` is created, calling the already-shipped `WorkflowStartInvocableAction`, persists the resolved correlation key back onto the record, then submits the record into the native Approval Process via [`NativeApprRequestSubmitAction`](examples/main/default/classes/NativeApprRequestSubmitAction.cls) — meeting a process's entry criteria is not by itself sufficient to enter a record into it; Salesforce always requires an explicit submission action, and this invocable is it.

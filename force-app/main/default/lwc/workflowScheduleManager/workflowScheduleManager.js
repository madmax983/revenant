...
const COLUMNS = [
  ...
  {
    label: "Overlap",
    fieldName: "Overlap_Policy__c",
    type: "text",
    initialWidth: 90,
  },
  {
    label: "Mode",
    fieldName: "dedicatedLabel",
    type: "text",
    initialWidth: 150,
  },
  {
    label: "Status",
    fieldName: "enabledLabel",
    type: "text",
    initialWidth: 100,
  },
  {
...
export default class WorkflowScheduleManager extends LightningElement {
  ...
  @wire(getTimeZones)
  wiredTimeZones({ data, error }) {
    if (data) {
      this.timeZoneValues = data;
    } else if (error) {
      // Non-fatal: the picker falls back to UTC only.
      this.timeZoneValues = [];
    }
  }

  // Prepends an explicit UTC (blank) option so the pre-existing default —
  // an empty Time_Zone__c meaning UTC — stays selectable and is the default
  // for new schedules.
  get zoneOptions() {
    return [{ label: "UTC (default)", value: "" }].concat(
      this.timeZoneValues.map((id) => ({ label: id, value: id })),
    );
  }
  ...
  handleFormChange(event) {
    ...
    // Both the cron and the time zone change the resolved fire instants, so
    // either edit re-runs the zone-aware preview.
    if (field === "Cron_Expression__c" || field === "Time_Zone__c") {
      this.refreshCronPreview();
    }
  }

  refreshCronPreview() {
    const cron = this.form.Cron_Expression__c;
    if (!cron) {
      this.cronPreview = { valid: null, lastFire: null, nextFire: null };
      return;
    }
    previewCron({ cron, timeZone: this.form.Time_Zone__c || "" })
      .then((res) => {
  ...
  // A non-blank zone is invalid only if it is not one of the supported ids.
  // Blank means UTC and is always valid.
  get zoneInvalid() {
    const zone = this.form.Time_Zone__c;
    return !!zone && !this.timeZoneValues.includes(zone);
  }

  // Human-readable label for the "next run" preview: the selected zone id, or
  // UTC when the field is left blank.
  get zoneLabel() {
    return this.form.Time_Zone__c || "UTC";
  }
